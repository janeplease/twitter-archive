...
public class GetLikesService {
    ...
    private static int numTotalLikes = 0;

    static HashSet<String> likedTweetsId;
    ...
    public static void writeDataInFile(Get2UsersIdLikedTweetsResponse response, String filename) {
        try {
            FileWriter fw = new FileWriter("D:\\Projects\\TwitterJavaSDKBasic2\\be\\data\\likes\\" + filename, true);
    ...
    public static Get2UsersIdLikedTweetsResponse processOneBatch(TwitterApi apiInstance, String userId, String paginationToken) {
        Integer maxResults = 100;
        int numberOfCalls = 0;
        Get2UsersIdLikedTweetsResponse result = null;
        AtomicInteger check = new AtomicInteger(0);
        do {
            try{
                numberOfCalls++;
                result = apiInstance.tweets().usersIdLikedTweets(userId)
                        .maxResults(maxResults)
                        .paginationToken(paginationToken)
                        .tweetFields(GetLikesService.getTweetFields())
                        .expansions(GetLikesService.getExpansions())
                        .mediaFields(GetLikesService.getMediaFields())
                        .pollFields(GetLikesService.getPollFields())
//                        .userFields(com.example.TwitterJavaSDKBasic2.GetLikes.getUserFields())
                        .placeFields(GetLikesService.getPlaceFields())
                        .execute();
                if(result!=null &&
                        result.getData()!=null)
                    numTotalLikes = numTotalLikes + result.getData().size();
                GetLikesService.writeDataInFile(result, "LikedTweets"+numTotalLikes + ".txt");
                System.out.println("Tweets liked till now is "+numTotalLikes);
                result.getData().forEach((tweet) -> {
                    if(!likedTweetsId.contains(tweet.getId())) {
                        likedTweetsId.add(tweet.getId());
                    }
                    else {
...
//                    break;
    ...
    public void getLikesByUserId(TwitterApi apiInstance, String userId) {

        DirectoryPreparation directoryPreparation = new DirectoryPreparation();
        directoryPreparation.clearDirectoryLikes();

        likedTweetsId = new HashSet<String>();

        Integer maxResults = 100; // Integer | The maximum number of results.

        Get2UsersIdLikedTweetsResponse result;
        ...
        System.out.println("Total number of tweets liked is "+numTotalLikes);
    }
}
